...
The current example selects the console's ROMVER-derived NTSC/PAL default in interlaced field mode, resets the GS and GIF, programs the CRT through the EE kernel syscall, configures `DISPFB2`/`DISPLAY2`, then draws a 32-bit framebuffer through packed GIF packets. It now exercises the GIF DMA path with a clipped solid rectangle and line before entering the audio/input smoke loop.
